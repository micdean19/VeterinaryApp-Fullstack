...
function App() {
    ...
    // Checks local storage (cache) and loads user from client side memory
    useEffect(() => {
    ...
    // Handle post request to database login-management
    // data is email/password entered by user
    const handleSubmit = (e) => {
        ...
        var config = {
            method: "post",
            // This is endpoint from Controller
            url: "http://localhost:8080/login-management",
            data: data,
        };

        // Send request to database using axios
        axios(config)
            .then((response) => {
    ...
    return (
        <Container fluid className="align-middle w-25">
            <div>
                ...
                {/* When login is pressed, submit calls handlsubmit function */}
